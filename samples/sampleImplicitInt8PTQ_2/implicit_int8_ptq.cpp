...
bool SampleINT8::build(DataType dataType)
{
    ...
    std::cout << "Input nbDims: " << network->getInput(0)->getDimensions().nbDims << std::endl;
    std::cout << " nbDims[2]: " << network->getInput(0)->getDimensions().d[2]
              << " nbDims[1]: " << network->getInput(0)->getDimensions().d[1]
              << " nbDims[0]: " << network->getInput(0)->getDimensions().d[0]
              << std::endl;
    ...
    SampleUniquePtr<nvcaffeparser1::ICaffeParser>& parser, DataType dataType)
{
    ...
    std::cout << "----------------------- [info] set int8 calibrator" << std::endl;
    if (dataType == DataType::kINT8)
    {
    ...
    std::cout << "----------------------- [info] set dla" << std::endl;
    ...
    std::cout << "----------------------- [info] set profile stream" << std::endl;
    ...
    std::cout << "----------------------- [info] build serialed network" << std::endl;
    SampleUniquePtr<IHostMemory> plan{builder->buildSerializedNetwork(*network, *config)};
    if (!plan)
    {
        return false;
    }
    std::cout << "----------------------- [info] creat infer runtime" << std::endl;
    SampleUniquePtr<IRuntime> runtime{createInferRuntime(sample::gLogger.getTRTLogger())};
    if (!runtime)
    {
        return false;
    }
    std::cout << "----------------------- [info] deserialize cuda engine" << std::endl;
    ...
    std::cout << "----------------------- [info] save engine" << std::endl;
...
{
    std::cout << "----------------------- [info] start infer" << std::endl;
    // return false;
    float ms{0.0f};
    ...
    if (!context)
    {
        std::cout << "[error] create execution context failed" << std::endl;
        return false;
    }
    ...
    while (batchStream.next())
    {
        ...
        {
            std::cout << "[error] process input failed" << std::endl;
            return false;
        }
        ...
        if (!status)
        {
            std::cout << "[error] enqueue failed" << std::endl;
            return false;
        }
...
SampleINT8Params initializeSampleParams(const samplesCommon::Args& args, int batchSize)
{
    ...
    params.dataDirs.emplace_back("data/MNIST/raw/");
...
int main(int argc, char** argv)
{
    ...
    for (size_t i = 0; i < dataTypes.size(); i++) {
      sample::gLogInfo << "========================>" << dataTypeNames[i]
                       << " run:" << nbScoreBatches << " batches of size "
                       << batchSize << " starting at " << firstScoreBatch
                       << std::endl;

      if (!sample.build(dataTypes[i])) {
        if (!samplesCommon::isDataTypeSupported(dataTypes[i])) {
          sample::gLogWarning
              << "Skipping " << dataTypeNames[i]
